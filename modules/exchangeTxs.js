...
module.exports = async (itx, tx) => {
	...
	if (inTxidDublicate){
		...
		pay.error = 1;
		notifyType = 'error';
		msgNotify = `Store.botName thinks transaction of _${inAmountMessage}_ _${inCurrency}_ is duplicated. Tx hash: _${inTxid}_. Will ignore this transaction. Income ADAMANT Tx: https://explorer.adamant.im/tx/${tx.id}.`;
		...
		pay.isFinished = true;
		notifyType = 'error';
		msgNotify = `Store.botName notifies about incoming transfer of unknown crypto: _${inCurrency}_. Attention needed. Income ADAMANT Tx: https://explorer.adamant.im/tx/${tx.id}.`;
		...
		msgNotify = `Store.botName notifies about request of unknown crypto: _${outCurrency}_. Will try to send payment back. Income ADAMANT Tx: https://explorer.adamant.im/tx/${tx.id}.`;
		...
		msgNotify = `Store.botName received request to exchange _${inCurrency}_ for _${outCurrency}_. Will try to send payment back. Income ADAMANT Tx: https://explorer.adamant.im/tx/${tx.id}.`;
		...
		msgNotify = `Store.botName notifies about incoming transfer of unaccepted crypto: _${inCurrency}_. Will try to send payment back. Income ADAMANT Tx: https://explorer.adamant.im/tx/${tx.id}.`;
		...
		msgNotify = `Store.botName notifies about incoming transfer of unaccepted crypto: _${outCurrency}_. Will try to send payment back. Income ADAMANT Tx: https://explorer.adamant.im/tx/${tx.id}.`;
		...
		if (userDailiValue + pay.inAmountMessageUsd >= config.daily_limit_usd){
			...
			msgNotify = `Store.botName notifies that user _${tx.senderId}_ exceeds daily limit of _${config.daily_limit_usd}_ USD with transfer of _${inAmountMessage} ${inCurrency}_. Will try to send payment back. Income ADAMANT Tx: https://explorer.adamant.im/tx/${tx.id}.`;
			...
			notifyType = 'warn';
			msgNotify = `Store.botName notifies about incoming transaction below minimum value of _${min_value_usd}_ USD: _${inAmountMessage}_ _${inCurrency}_. Will try to send payment back. Income ADAMANT Tx: https://explorer.adamant.im/tx/${tx.id}.`;
	...
	if (!pay.isFinished && !pay.needToSendBack){// if Ok checks tx
		...
		if (!pay.outAmount){ // Error while calculating outAmount
			...
			notifyType = 'warn';
			msgNotify = `Store.botName unable to calculate _outAmount_ value. Will try to send payment back. Income ADAMANT Tx: https://explorer.adamant.im/tx/${tx.id}.`;
			...
			notifyType = 'log';
			msgNotify = `Store.botName notifies about incoming transaction to exchange _${inAmountMessage}_ _${inCurrency}_ for *${pay.outAmount}* *${outCurrency}* at _${pay.exchangePrice}_ _${outCurrency}_ / _${inCurrency}_. Tx hash: _${inTxid}_. Income ADAMANT Tx: https://explorer.adamant.im/tx/${tx.id}.`;
