...
module.exports = async (pay, tx) => {
	...
	try {
		...
		if (senderKvsInAddress === 'none') {
			...
			notifyType = 'error';
			notify(`Store.botName cannot fetch address from KVS for crypto: _${pay.inCurrency}_. Income ADAMANT Tx: https://explorer.adamant.im/tx/${tx.id}. Attention needed.`, 'error');
		...
		if (senderKvsOutAddress === 'none' && !pay.needToSendBack) {
			...
			notifyType = 'warn';
			msgNotify = `Store.botName cannot fetch address from KVS for crypto: _${pay.outCurrency}_. Will try to send payment back.`;
		...
		try {
			...
			if (!in_tx) {
				...
				notifyType = 'warn';
				msgNotify = `Store.botName can’t fetch transaction of _${pay.inAmountMessage} ${pay.inCurrency}_.`;
				...
				if (pay.sender.toLowerCase() !== pay.senderKvsInAddress.toLowerCase()) {
					...
					notifyType = 'warn';
					msgNotify = `Store.botName thinks transaction of _${pay.inAmountMessage}_ _${pay.inCurrency}_ is wrong. Amount expected: _${pay.inAmountMessage}_, but real amount is _${pay.inAmountReal}_.`;
					msgSendBack = `I can’t validate transaction of _${pay.inAmountMessage}_ _${pay.inCurrency}_ with Tx ID _${pay.inTxid}_. If you think it’s a mistake, contact my master.`;
