		...
		try {
			...
			if (status === false) {
				...
				if (type === 'exchange') {
					...
					msgNotify = `Exchange Bot ${Store.botName} sent back of _${inAmountMessage} ${inCurrency}_ failed. Tx hash: _${sendTxId}_. Will try again. Balance of _${sendCurrency}_ is _${Store.user[sendCurrency].balance}_. ${etherString}Income ADAMANT Tx: _https://explorer.adamant.im/tx/${admTxId}_.`;
				...
				notifyType = 'info';
				if (type === 'exchange') {
					msgNotify = `Exchange Bot ${Store.botName} successfully exchanged _${inAmountMessage} ${inCurrency}_ for _${outAmount} ${outCurrency}_ with Tx hash: _${sendTxId}_. Income ADAMANT Tx: _https://explorer.adamant.im/tx/${admTxId}_.`;
					msgSendBack = 'Done! Thank you for business. Hope to see you again.';

				} else { // type === 'back'
					msgNotify = `Exchange Bot ${Store.botName} successfully sent back _${inAmountMessage} ${inCurrency}_ with Tx hash: _${sendTxId}_. Income ADAMANT Tx: _https://explorer.adamant.im/tx/${admTxId}_.`;
